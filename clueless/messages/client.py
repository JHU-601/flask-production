...
class Register:
    def __init__(self, character: Union[Character, int], display_name: str):
        self.character = character if isinstance(character, Character) else Character(character)
...
class Suggest:
    def __init__(self, room: Union[Room, int], weapon: Union[Weapon, int], suspect: Union[Character, int]):
        self.room = room if isinstance(room, Room) else Room(room)
        self.weapon = weapon if isinstance(weapon, Weapon) else Weapon(weapon)
        self.suspect = suspect if isinstance(suspect, Character) else Character(suspect)
...
class SuggestionResponse:
    def __init__(self, witness: Optional[Union[Character, Room, Weapon]], type: Union[WitnessType, int] ):
        self.type = type if isinstance(type, WitnessType) else WitnessType(type)
        if isinstance(witness, Character) or isinstance(witness, Room) or isinstance(witness, Weapon):
            ...
            if self.type == WitnessType.ROOM:
                self.witness = Room(witness)
            elif self.type == WitnessType.CHARACTER:
                self.witness = Character(witness)
            else:
                self.witness = Weapon(witness)
