QUnit.module('Position', function() {
  ...
  QUnit.test('can move upleft', function(assert) {
    let obj = new Position(Room.KITCHEN);
    let next = obj.getRelativePosition('upleft');
    assert.equal(next.id, Room.STUDY);
    next = next.getRelativePosition('upleft');
    assert.equal(next, null);
  });
  QUnit.test('can move upright', function(assert) {
    let obj = new Position(Room.CONSERVATORY);
    let next = obj.getRelativePosition('upright');
    assert.equal(next.id, Room.LOUNGE);
    next = next.getRelativePosition('upright');
    assert.equal(next, null);
  });
  QUnit.test('can move downleft', function(assert) {
    let obj = new Position(Room.LOUNGE);
    let next = obj.getRelativePosition('downleft');
    assert.equal(next.id, Room.CONSERVATORY);
    next = next.getRelativePosition('downleft');
    assert.equal(next, null);
  });
  QUnit.test('can move downright', function(assert) {
    let obj = new Position(Room.STUDY);
    let next = obj.getRelativePosition('downright');
    assert.equal(next.id, Room.KITCHEN);
    next = next.getRelativePosition('downright');
    assert.equal(next, null);
  });
});
